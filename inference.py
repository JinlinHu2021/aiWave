...
with tf.Session() as sess:
    saver.restore(sess, '/data/xiaoban/EM_image_small_raw/add_4/my-model-700')

    for batch_index in range(1):

        path1 = '/data/xiaoban/EM_image_small_raw/test_mri_new'
        f1 = os.listdir(path1)
        for k in range(0, len(f1)):

            print('img_ID:', k + 1)

            tif = load_nii(os.path.join('/data/xiaoban/EM_image_small_raw/test_mri_new', f1[k]))
